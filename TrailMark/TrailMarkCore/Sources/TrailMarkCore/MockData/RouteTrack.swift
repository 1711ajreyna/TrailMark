...
extension RouteTrack {
    public static let mockStartDate =
        Date(timeIntervalSince1970: 1_785_983_510)
    ...
    public static func mock(
        ...
        let safeSamplesPerSegment = max(samplesPerSegment, 1)

        var points: [TrackPoint] = []

        // This gives us an array with N-1 segments.
        for (segmentIndex, origin) in
            waypoints.dropLast().enumerated() {

            let destination = waypoints[segmentIndex + 1]

            /*
             Because dropLast() creates one fewer segment than
             the waypoint count, the last segment index is count - 2.
             */
            let isFinalSegment =
                segmentIndex == waypoints.count - 2

            let stepCount = isFinalSegment
                ? safeSamplesPerSegment
                : safeSamplesPerSegment - 1

            for steps in 0...stepCount {
                let progress =
                    Double(steps) /
                    Double(safeSamplesPerSegment)
